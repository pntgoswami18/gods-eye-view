...
- **Google Maps Platform — one key isn't enough once you restrict it.** `GOOGLE_MAPS_API_KEY` alone is enough for the 3D globe (Map Tiles API), but the location search bar and Places-backed context (`/api/google/geocode`, `/api/google/nearby-places`, `/api/google/text-search` in `vite.config.js`) need Geocoding API and Places API (New) enabled too — and, if you HTTP-referrer-restrict that key (recommended, since it's client-exposed), those two APIs stop working entirely: Google's Geocoding API refuses referrer-restricted keys outright, and Places API (New) enforces its referrer check even on a server-side call carrying no browser Referer. The fix is a second, server-side-only `GOOGLE_SERVER_API_KEY` (see `.env.example`) — Application restrictions **None** (or IP, if you have a static egress IP), API restrictions limited to Geocoding API + Places API (New), and never sent to the browser. Symptom without it: the globe works, search returns "Location not found" with a `REQUEST_DENIED`/403 visible in devtools.
