...
- **NYC DOT TMC cameras.** The camera list and per-camera still frames both come from the keyless `webcams.nyctmc.org` endpoint (no key or rate-limit registration needed). Only `isOnline: "true"` cameras are kept, and the loader pins fetched image URLs to the official host before serving them through the CCTV proxy.
