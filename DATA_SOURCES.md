...
| **NYC DOT (webcams.nyctmc.org)** | CCTV camera catalog + frames, New York City | Public NYC DOT traffic camera data | "NYC DOT — webcams.nyctmc.org" (courtesy) |
| **City of Toronto Open Data** | CCTV camera catalog + frames, Toronto | City of Toronto Open Data license (not specified — public traffic camera data) | "City of Toronto — open.toronto.ca" (courtesy) |
| **Seattle DOT (Seattle GeoData)** | CCTV camera catalog + frames, Seattle | Public Seattle DOT traffic camera data | "Seattle DOT — Seattle GeoData" (courtesy) |
...
- **NYC DOT TMC cameras.** The camera list and per-camera still frames both come from the keyless `webcams.nyctmc.org` endpoint (no key or rate-limit registration needed). Only `isOnline: "true"` cameras are kept, and the loader pins fetched image URLs to the official host before serving them through the CCTV proxy. Note: the frame host applies bot/network-level protections that can reject requests from some hosting networks — verify reachability from your own deployment before relying on this pack.
- **Toronto TMC cameras.** The camera list comes from a keyless JSONP endpoint on `opendata.toronto.ca` (no key needed); frames are built from the camera number against `www.toronto.ca`'s own image host, a separate origin from the catalog. The catalog carries no per-camera online/offline flag, so every listed camera is treated as live.
- **Seattle SDOT cameras.** One keyless ArcGIS FeatureServer query on `services.arcgis.com` (Seattle's public GIS org) returns the catalog with an absolute per-camera image URL already populated; only `SERVSTAT: "ACTV"` cameras are kept. Note: the frame host (`www.seattle.gov`) applies bot/network-level protections that can reject requests from some hosting networks — verify reachability from your own deployment before relying on this pack.
