...
- **Google Maps Platform.** You supply your own API key and are bound by [Google's ToS](https://cloud.google.com/maps-platform/terms). Google Maps Content (tiles, geocodes, places) **may not be cached, stored, rehosted, or committed** — this app only ever uses it live, which is the compliant pattern. The "Google" attribution is displayed on the globe and must stay visible. Restrict your key (see [SECURITY.md](SECURITY.md)). Note the key needs more than one API enabled depending on which features you want: **Map Tiles API** alone is enough for the photorealistic 3D globe to render, but the location search bar (client-side `maps.googleapis.com/maps/api/geocode`, see `src/locations.js`) needs **Geocoding API**, and nearby-places context / text-search-driven features (`/api/google/nearby-places`, `/api/google/text-search`) need **Places API (New)**. Without those enabled — or added to the key's API restrictions if you've scoped it — the globe still works but search and Places-backed features fail with `REQUEST_DENIED` / a 403, visible in devtools.
